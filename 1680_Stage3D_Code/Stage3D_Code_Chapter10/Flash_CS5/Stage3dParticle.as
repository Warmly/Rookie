...
public class Stage3dParticle extends Stage3dEntity
{
	...
	// not multiple times, once for each particle
	private static const particleshader1mesh:Program3D = null;
	private static const particleshader2mesh:Program3D = null;
		...
		scale1:Number = -999, scale2:Number = -999):void
	{
		...
		// to ignore everything in the matrix but the pos:
		//x = pos.position.x;
		//y = pos.position.y;
		//z = pos.position.z;
		...
		rgbaScale[2] = 1;
		rgbaScale[3] = 1;
		if (scale1 != -999) startSize = scale1;
		if (scale2 != -999) endSize = scale2;
		active = true;
		...
		statechanged:Boolean = true):void
	{
		...
		//trace('rendering particle with scale ' + scaleX);
		//trace('rendering particle at ' + transform.position);
		...
		// other meshes may have set these
